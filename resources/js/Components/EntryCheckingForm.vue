<template>
    <v-container>
        <v-card :loading="isLoading" class="pa-5" @keydown.ctrl.alt.space="showLastTransaction">
            <v-card-text>
                <v-form ref="entryForm" v-model="isValid" aria-label="$t('entry_check_form')" validate-on="input lazy"
...
export default {
    ...
    computed: {
        ...
        resultClass() {
            return this.show
                ? (this.result.success ? 'text-green' : 'text-red')
                : '';
        },
        ...
        rules() {
            return {
                academic_id: [
                    ...
                    v => v !== this.lastTransaction.id || this.isViewingLastTransaction || !this.show || this.$t('validation.entry_same_user'),
                ],
            };
        },
    },
