<template>
    <div v-if = "applicationEdit">
        ...
        <div v-for = "(file, index) in files" :key = "index" class = "mt-3">
            ...
            <input
                v-if = "canEditDocument[index]" accept = "application/pdf" type = "file"
                v-bind:disabled = "!file.description" @change = "onFileChange($event, index)"
            >
            <input
                v-model = "file.description" :placeholder = "$t('description')" class = "form-control"
            ...
            <div class = "row justify-content-between ms-auto me-auto">
                ...
                <template v-if = "canEditDocument[index]">
                    <button
                        v-if = "file.id !== 0" class = "btn btn-danger col-auto "
			...
			return await this.$axios.post( url, params )
			                 ...
			                 .finally( function () {
				                 ...
				                 if ( file.result.success ) {
					                 vue.docFiles[ index ].status = file.status = ( "to delete" === file.status )
					                                                              ? "deleted"
					                                                              : "submitted";
