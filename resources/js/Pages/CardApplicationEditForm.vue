        ...
        <div class = "row ">
            <div class = "col-5">
                ...
                <header>
                    ...
                    <button
                        v-if = "status === $enums.CardStatusEnum.TEMPORARY_SAVED" :aria-label = "$t('save')"
                    ...
                    <button
                        v-else-if = "applicationEdit" :aria-label = "$t('edit')" class = "btn btn-secondary"
                        @click = "changeToEdit()"
                    >
                ...
                <form
                    v-if = "status === $enums.CardStatusEnum.TEMPORARY_SAVED" id = "card_application_form"
                    v-on:submit.prevent = ""
                >
                    <!-- Show card documents -->
                    <CardDocumentsShow
                        ref = "CardDocuments" v-bind:applicationEdit = "true"
	...
	computed : {
		...
		status :          function () {
			return this.cardApplication
			       ? this.cardApplication.card_last_update.status
			       : null;
		},
		expiration_date : function () {
			return this.lastExpiration || ( this.cardApplication
			                                ? this.cardApplication.expiration_date
			                                : null );
		},
			...
			this.$axios
			    ...
			    .then( responseJson => {
				    ...
				    this.result.message = ( json[ "success" ] )
					    ...
					    "from1" : `status.${ this.cardApplication.card_last_update.status.replace(
						    " ", "_" ) }`,
